...
import { type DynamicRegistrationOptions, registerDynamicClient } from './dynamic.ts';

function options(enableWriteScope = false): DynamicRegistrationOptions & { repos: ReturnType<typeof openTestRepos> } {
...
describe('registerDynamicClient', () => {
  it('OAUTH-11 mints a vg_c_ id, echoes the metadata and stores the client', () => {
    ...
    const response = unwrapOk(
      registerDynamicClient({ ...MINIMAL, client_name: ' Agent ', scope: 'vault:read', ignored: 1 }, deps),
    );
    ...
    expect(deps.repos.clients.findByClientId(response.client_id)).toStrictEqual({
      id: 'row-1',
  ...
  it('OAUTH-5 rejects confidential authentication methods with invalid_client_metadata', () => {
    const error = unwrapFail(
      registerDynamicClient({ ...MINIMAL, token_endpoint_auth_method: 'client_secret_basic' }, options()),
    );
  ...
  it('OAUTH-6 rejects a bad redirect URI with invalid_redirect_uri', () => {
    const error = unwrapFail(registerDynamicClient({ redirect_uris: ['http://app.example.com/cb'] }, options()));
    expect(error.code).toBe('invalid_redirect_uri');
    expect(unwrapFail(registerDynamicClient({ redirect_uris: [] }, options())).code).toBe('invalid_redirect_uri');
    expect(unwrapFail(registerDynamicClient({}, options())).code).toBe('invalid_redirect_uri');
  });
  ...
  it.each([
    ...
    const error = unwrapFail(registerDynamicClient({ ...MINIMAL, ...extra }, options()));
    expect(error.code).toBe('invalid_client_metadata');
    expect(error.description.startsWith(`${path}:`)).toBe(true);
  });

  it('OAUTH-11 rejects a non-object body', () => {
    expect(unwrapFail(registerDynamicClient('text', options())).description).toBe('the body must be a JSON object');
    expect(unwrapFail(registerDynamicClient(null, options())).code).toBe('invalid_client_metadata');
  });

  it('OAUTH-16 rejects unknown scopes and vault:write while it is disabled', () => {
    expect(unwrapFail(registerDynamicClient({ ...MINIMAL, scope: 'vault:read vault:admin' }, options())).description).toBe(
      'scope "vault:admin" is not available',
    );
    expect(unwrapFail(registerDynamicClient({ ...MINIMAL, scope: 'vault:write' }, options())).description).toBe(
      'scope "vault:write" is not available',
    );
    expect(unwrapOk(registerDynamicClient({ ...MINIMAL, scope: 'vault:write' }, options(true))).scope).toBe('vault:write');
    expect(unwrapOk(registerDynamicClient({ ...MINIMAL, scope: '' }, options())).scope).toBe('');
  });
