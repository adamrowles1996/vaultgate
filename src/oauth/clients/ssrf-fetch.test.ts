...
describe('safeFetch', () => {
  it('OAUTH-8 fetches with Accept: application/json, manual redirects and a timeout', async () => {
    ...
    expect(url).toBe('https://agent.example.com/c.json');
    expect(init.method).toBe('GET');
    expect(init.redirect).toBe('manual');
    expect(init.headers).toStrictEqual({ Accept: 'application/json' });
  ...
  it('OAUTH-8 follows at most two redirects, re-validating each hop', async () => {
    ...
    const fetch: FetchLike = (url) =>
      Promise.resolve(hops.get(url) ?? new Response('final', { status: 200 }));
