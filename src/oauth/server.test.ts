...
describe('createAuthorizationServer', () => {
  it('OAUTH-12 fails on an invalid pre-registered client list', () => {
    const result = createAuthorizationServer({
      config: {
        ...
        oauthClients: [{ clientId: 'bad', clientName: undefined, redirectUris: ['http://evil.example.com/cb'] }],
        accessTokenTtlMs: 1,
        refreshTokenTtlMs: 1,
        trustProxy: false,
      },
  ...
  it('OAUTH-12 persists valid pre-registered clients at start-up', () => {
    const harness = createOAuthHarness({
      oauthClients: [{ clientId: 'desk', clientName: 'Desk', redirectUris: ['https://desk.example.com/cb'] }],
    });
    expect(harness.repos.clients.findByClientId('desk')).toMatchObject({ mode: 'preregistered', clientName: 'Desk' });
  });
  ...
  it('OAUTH-30 exposes consent revocation and the connected-client list for the account page', () => {
    ...
    expect(harness.server.listConnectedClients(OPERATOR_ID).map((client) => client.id)).toStrictEqual(['consent-1']);
    ...
    expect(harness.audit.at(-1)).toMatchObject({ action: 'consent_revoked', operatorId: OPERATOR_ID, clientId: 'vg_c_x' });
  });
});
