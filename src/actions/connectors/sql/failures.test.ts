...
describe('driver faults', () => {
  it('ACT-74 reads the code of an error and of every error it wraps', () => {
    ...
    expect(fault).toStrictEqual({ codes: ['ESOCKET', 'ECONNREFUSED'], message: 'outer' });
  });
  ...
  it('ACT-74 an error with no code and a value that is not an error still yield a message', () => {
    expect(faultOf(new Error('plain'))).toStrictEqual({ codes: [], message: 'plain' });
    expect(faultOf('not an error')).toStrictEqual({ codes: [], message: 'not an error' });
  });
