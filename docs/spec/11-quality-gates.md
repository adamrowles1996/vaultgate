...
| Workflows           | actionlint (with shellcheck over every `run:` step)                                                                                                               | clean                                                                                  |
| Shell               | shellcheck, shfmt                                                                                                                                                 | clean, formatted                                                                       |
| Markdown            | markdownlint-cli2                                                                                                                                                 | clean (line length is Prettier's concern)                                              |
| Spelling            | cspell, en-GB, project dictionary in `cspell.json`                                                                                                                | zero unknown words in code, comments and prose                                         |
| Regular expressions | eslint-plugin-regexp                                                                                                                                              | no malformed or super-linear patterns (ReDoS)                                          |
| Node usage          | eslint-plugin-n                                                                                                                                                   | `node:` protocol, no deprecated APIs, `process.exit` only in `main.ts` and `cli.ts`    |
| Modern JavaScript   | eslint-plugin-unicorn (curated)                                                                                                                                   | kebab-case filenames, contextual error messages, no legacy idioms                      |
| Test hygiene        | @vitest/eslint-plugin                                                                                                                                             | no focused or skipped tests, every test asserts, no conditional expects                |
| Module graph        | dependency-cruiser (`.dependency-cruiser.mjs`)                                                                                                                    | layering rules from §02.2, no orphans, no test code or dev deps in runtime             |
| Lockfile            | lockfile-lint                                                                                                                                                     | npm registry over https only, integrity hashes present                                 |
| Manifest            | sort-package-json                                                                                                                                                 | deterministic key order                                                                |
| Whitespace          | editorconfig-checker                                                                                                                                              | every tracked file honours `.editorconfig`                                             |
| Secrets             | gitleaks (history in CI, staged files in the pre-commit hook)                                                                                                     | none, including vaultgate's own `vg_*` credential shapes                               |
| Package provenance  | `npm audit signatures`                                                                                                                                            | every installed package carries a valid registry signature                             |

## 11.2 Test gates

| Gate               | Tool                                                                        | Threshold                                                                                                   |
| ------------------ | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Unit + integration | vitest                                                                      | 100% statements, branches, functions and lines over `src/**` (entrypoints excluded, covered by smoke)       |
| Boot smoke         | `scripts/smoke-test.sh`                                                     | compiled build and type-stripped source both answer `/healthz`                                              |
| CLI smoke          | `scripts/cli-smoke.sh`                                                      | compiled build and type-stripped source both export an empty audit log (JSON Lines, CSV header) with exit 0 |
...
- **QG-4** Exact dependency versions (`save-exact`), committed lockfile, `npm ci` everywhere.
  Non-npm linters are pinned in `.mise.toml`; `mise install` reproduces CI's toolchain locally.
